#!/usr/bin/env python3
"""Actual target-process force-stop with a committed location queue; not a kill during commit or hardware test."""
import argparse
import os
...
def main():
    ...
    command = [*adb, 'shell', 'am', 'instrument', '-w', '-r', '-e', 'class', 'app.umbra.DeviceSignalTest',
               '-e', 'listener', 'app.umbra.LocationRestartFixtureListener', '-e', 'locationPhase']
    runner = package + '.test/androidx.test.runner.AndroidJUnitRunner'
    args.log_dir.mkdir(parents=True, exist_ok=True)
    before = args.log_dir / (args.flavor + '-location-before-kill.log')
    with before.open('w') as stream:
        process = subprocess.Popen([*command, 'prepare', runner], stdout=stream, stderr=subprocess.STDOUT)
        try:
            deadline = time.monotonic() + 45
            while 'locationRestart=READY' not in before.read_text():
                if process.poll() is not None or time.monotonic() >= deadline:
                    raise RuntimeError('Location prepare failed or timed out; inspect ' + str(before))
                time.sleep(0.1)
    ...
    after = args.log_dir / (args.flavor + '-location-after-kill.log')
