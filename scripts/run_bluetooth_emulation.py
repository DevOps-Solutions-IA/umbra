...
def main() -> None:
    ...
    try:
        ...
        for text in texts():
            if ('nearbyResult=PASS:' not in text or not re.search(r'^OK \(3 tests\)$', text, re.MULTILINE)
        ...
        successful = True
        print(f'PASS: actual emulated Bluetooth RFCOMM on {devices}; both directions, host code comparison, text, attachment, duplicate and receipts. Not physical Bluetooth or Vault persistence.')
    finally:
        for process in processes:
