#!/usr/bin/env python3
"""Install synthetic debug fixtures and require all eight real single-device checks."""
...
def main() -> None:
    ...
    failed = result.returncode != 0 or not re.search(r'^OK \(8 tests\)$', output, re.MULTILINE)
    ...
    if failed:
        raise SystemExit(f'Instrumentation failed, skipped checks, or did not run exactly 8 tests: {args.log}')
    print(f'{args.flavor}: 8 Android instrumentation tests passed on {args.serial}; log: {args.log}')
