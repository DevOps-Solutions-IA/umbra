#!/usr/bin/env python3
"""Install synthetic debug fixtures and require all sixteen real single-device checks."""
...
def main() -> None:
    ...
    failed = result.returncode != 0 or not re.search(r'^OK \(16 tests\)$', output, re.MULTILINE)
    ...
    if failed:
        raise SystemExit(f'Instrumentation failed, skipped checks, or did not run exactly 16 tests: {args.log}')
    print(f'{args.flavor}: 16 Android instrumentation tests passed on {args.serial}; log: {args.log}')
