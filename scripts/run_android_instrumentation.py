#!/usr/bin/env python3
"""Install synthetic debug fixtures and require all twenty-five real single-device checks."""
...
def main() -> None:
    ...
    failed = result.returncode != 0 or not re.search(r'^OK \(25 tests\)$', output, re.MULTILINE)
    ...
    if failed:
        raise SystemExit(f'Instrumentation failed, skipped checks, or did not run exactly 25 tests: {args.log}')
    print(f'{args.flavor}: 25 Android instrumentation tests passed on {args.serial}; log: {args.log}')
