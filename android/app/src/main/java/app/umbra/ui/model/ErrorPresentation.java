...
public record ErrorPresentation(ErrorKind kind, String title, String body, Glyph glyph, Tone tone,
    ...
    public static ErrorPresentation of(ErrorKind kind, String technical) {
        ...
        return switch (kind) {
            ...
            case IDENTITY_CHANGED -> new ErrorPresentation(kind, "La identidad cambió", "La identidad criptográfica de este contacto cambió. Verifica nuevamente antes de continuar con operaciones sensibles.", Glyph.WARNING, Tone.IDENTITY, false, t);
            case CONTACT_BLOCKED -> new ErrorPresentation(kind, "Contacto bloqueado", "La operación se detuvo porque el contacto está bloqueado.", Glyph.BLOCK, Tone.BLOCKED, false, t);
            case DEVICE_REVOKED -> new ErrorPresentation(kind, "Dispositivo revocado", "Este dispositivo ya no está autorizado. Las operaciones con él se bloquearon.", Glyph.BLOCK, Tone.BLOCKED, false, t);
